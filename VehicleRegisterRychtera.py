...
class Register:
    ...
    def update_vehicle_owner(self, registration_plate: str, new_owner: Person) -> int:
        status = 0
        for each_vehicle in self.car_register:
            if each_vehicle.registration_plate == registration_plate:
                if each_vehicle.owner != new_owner:
                    each_vehicle.owner = new_owner
                    status = 1
        self.delete_owners()
        return status
